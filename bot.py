...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if name:
        await update.message.reply_text(
            ...
            "Команди:\n"
            "/report — отримати Excel з ЗП за місяць\n"
            "/clear — почати місяць заново\n"
            "/name — змінити ім'я\n"
...
async def handle_date(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        rate_msg + "\n\nЄ складські товари (червоні)? Введи артикули через кому або надішли /no",
        parse_mode="Markdown"
    )
    return WAIT_STOCK
...
async def handle_stock(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() not in ["/no", "ні", "нет", "no", "/no"]:
    ...
    await update.message.reply_text(
        ...
        f"Надішли наступний PDF або /report для отримання Excel.",
        parse_mode="Markdown"
    )
    return ConversationHandler.END
